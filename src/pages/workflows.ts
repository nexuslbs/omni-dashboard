...
function toolsetOptions(current: string): string {
  const cur = current || "";
    ...
    return `
      <details class="wf-role-details" open style="margin-bottom:.6rem;border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:.6rem .8rem;">
        ...
        <div class="wf-role-fields ${enabled ? "" : "wf-role-fields-disabled"}" data-role="${role}" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:.6rem;margin-top:.6rem;">
          ...
          <label style="display:flex;flex-direction:column;font-size:.88rem;color:#99a;">Toolset
            <select id="wf-${role}-toolset" class="filter-select wf-role-toolset" data-role="${role}">
              ${toolsetOptions(cfg.toolset || "")}
            </select>
          </label>
        </div>
      </details>`;
    ...
    <div class="card" style="margin-bottom:1rem;">
      ...
      <div class="card-body">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:.6rem;margin-bottom:.75rem;">
          ...
          <label style="display:flex;flex-direction:column;font-size:.88rem;color:#99a;">Toolset
            <select id="wf-toolset" class="filter-select">
              ${toolsetOptions(wf.toolset || "")}
            </select>
          </label>
...
function collectRole(role: string): WorkflowRoleConfig {
  ...
  const toolset =
    document.querySelector<HTMLSelectElement>(`.wf-role-toolset[data-role="${role}"]`)?.value ?? "";
  ...
  if (toolset) cfg.toolset = toolset;
  return cfg;
}
...
async function handleSave(): Promise<void> {
  ...
  const toolset = (document.getElementById("wf-toolset") as HTMLSelectElement | null)?.value ?? "";
  ...
  if (toolset) workflow.toolset = toolset;
  if (retriesRaw !== "") {
