...
function renderActionRow(a: Action, i: number): string {
  ...
  return `<tr class="${isDisabled ? "action-disabled" : ""}">
    <td${isDisabled ? ' style="opacity:0.55"' : ""}><strong>${escapeHtml(a.name)}</strong>${isDisabled ? ' <span class="badge badge-neutral" style="font-size:0.7rem;background:rgba(245,158,11,0.15);color:#f59e0b;border:1px solid rgba(245,158,11,0.3);padding:0.05rem 0.35rem;border-radius:4px;margin-left:0.35rem;vertical-align:middle">Disabled</span>' : ""}</td>
    <td${isDisabled ? ' style="opacity:0.55"' : ""}><code>${toolDisplay}</code></td>
    <td style="font-size:0.8rem;color:var(--text-muted);max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap${isDisabled ? ";opacity:0.55" : ""}">${paramsStr}</td>
